...
- **Image-based deployment**: A deployment type (MCP, Interceptor, Adapter) that references an `ImageDefinition` to determine the container image. Carries `imageDefinitionId`, `imageDefinitionName`, `imageDefinitionVersion`.
...
#### Scenario: Create image-based deployment
...
├── ImageBasedDeploymentDto (abstract) — adds imageDefinitionId, imageDefinitionName, imageDefinitionVersion
│   ├── McpDeploymentDto
