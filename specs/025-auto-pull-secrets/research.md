...
**Rationale**: The matching logic already exists and is tested implicitly via image inspection — `DockerRegistryClient.getRegistryClient()` (`docker/DockerRegistryClient.java:111-154`) matches against the primary registry (`registryProperties.getAuth()==BASIC && DockerHubAliases.sameRegistry(...)`) and iterates `trustedPrivateRegistries` checking `authScheme=="BASIC"` plus non-null user/password. Reusing it keeps the host-matching predicate identical to the inspect-time match, and keeps pure-config logic in `service/` (constitution I/III — no K8s here). Precedence, however, intentionally diverges since the post-release fix: deploy-time resolution checks trusted-private registries before the primary registry (least-privilege pull account — see the D2 amendment), while inspection and builds keep preferring the primary registry.
...
**Amendment (post-release fix)**: The auths keys in this document must be **bare registry hosts** (port preserved, no scheme, no `/v2` path), except Docker Hub which uses the legacy `https://index.docker.io/v1/` index key. The build-time `config.json` shape (`https://<host>/v2`, produced by `RegistryService.authKey`) is tolerated by docker CLI / BuildKit / skopeo, which normalize keys to a hostname — but the kubelet credential keyring path-matches keys against the image reference, so a `/v2`-decorated key never matches a `host/image:tag` pull and pods fail with `no basic auth credentials`. `RegistryService.dockerConfigForImages` therefore keys entries with `pullAuthKey` (bare host), deliberately diverging from `dockerConfig()`'s build-oriented `authKey`. The same fix also flipped deploy-time credential precedence: when one host is configured both as the primary registry and as a trusted-private registry, the pull secret carries the **trusted-private** credentials (least-privilege — e.g. a read-only pull account) rather than the primary registry's write-capable account; inspection (`DockerRegistryClient`) and builds (`dockerConfig()`) keep preferring the primary registry.
