...
**Status: Implemented.** Production code, unit tests, Checkstyle, and the full fast suite (`./gradlew testFast`, H2) are green. Three deviations from the original task wording, all documented in `research.md`:

- **Injection seam moved out of the generators (D4).** `imagePullSecrets` is injected onto the already-built CRD inside the deployment managers (`KnativeDeploymentManager.applyImagePullSecret`, `InferenceDeploymentManager.applyTransformerImagePullSecret`) rather than by threading a parameter through `serviceConfig(...)`/`apply(...)`. So T008/T013 (new mapper fields) and T009/T014 (generator-signature changes) were **not** needed — the manifest generators, `utils/mapping/*Mappers`, and their unit tests are unchanged. `RevisionSpec` already exposes `setImagePullSecrets`; the KServe `Transformer` exposes it via the generated `…transformer.ImagePullSecrets` type.
- **Secret content narrowed (D2).** T002/T003's `ResolvedRegistryCredential` + `resolveForImage(...)` were superseded by a single `RegistryService.dockerConfigForImages(Collection<String>)` that returns a `{"auths":{…}}` document containing ONLY the registries serving the deployment's own images (least-privilege). `RegistryService.dockerConfig()` (build-time, all registries) is left unchanged.
- **Test coverage is at the deployment-manager + service level.** The pull-secret-present/absent assertions (originally T011/T016, US1/US2) live in `KnativeDeploymentManagerTest` / `InferenceDeploymentManagerTest` (`deploy_shouldInjectImagePullSecret…`). US3 guardrails (T018–T022) and the narrowing/alias/anonymous/flag-off cases are covered by `RegistryServiceTest.dockerConfigForImages_*` and `RegistryPullSecretProvisionerTest`. Dedicated end-to-end H2 functional deploy tests (T012/T017) and the full testcontainers `clean build` (T025) were not added in this pass — the fast suite + manager-level tests cover the behaviour; a follow-up may add full functional tests.
