...
4. **Given** a deployment configured to scale to zero that has idled down to zero replicas, **When** a request wakes it (and after any intervening redeploys), **Then** the pull secret its revision references still exists, the cold-start pod pulls the image, and the deployment auto-resumes without any manual stop/start.
...
- **Scale-to-zero auto-resume** (issue #387): A deployment configured to scale to zero idles down to zero replicas and is later woken by a request. The pull secret its (immutable, still-live) serverless revision references must still exist so the cold-start pod can pull its image. Provisioning must therefore not rename/replace the referenced secret on redeploy in a way that orphans it, and the scheduled cleaner must not reclaim a secret a scaled-to-zero revision still references — otherwise auto-resume fails silently (the pod never starts, so no logs/events appear) and only a manual stop/start temporarily recovers it.
...
- **FR-016**: The auto-provisioned pull secret MUST have a **stable, deterministic per-deployment name** and MUST be provisioned via an **idempotent create-or-replace**. Redeploying a deployment MUST update the existing secret in place (rewriting its credentials) rather than creating a differently-named secret and orphaning the previous one. *(Fixes #387 — see the scale-to-zero edge case; the original implementation minted a fresh random-suffixed name on every deploy.)*
- **FR-016a**: The pull secret MUST be written to the cluster in the same post-commit phase as the generated CRD (not before the deploy transaction commits), mirroring how the network policy is applied — so a rolled-back or failed deploy never leaves a mutated pull secret stranded from the workload that references it. Within that phase the ordering is: provision the secret **before** the CRD apply (so the new revision's pods can pull), and condemn stale/legacy secrets only **after** the CRD apply succeeded — a failed apply must never condemn a secret the still-live revision references; the worst case of a failure in between is a leaked secret, reclaimed on the next successful redeploy. A condemnation failure after a successful CRD apply is tolerated (logged) rather than failing the deploy.
- **FR-017**: The system MUST NOT delete or reclaim a pull secret while any live **or scaled-to-zero** workload revision still references it. Because serverless (Knative) revisions are immutable and a scaled-to-zero revision keeps the secret name baked in, resource-lifecycle cleanup MUST NOT reclaim the name the currently-referenced revision depends on. This makes the "never remove credentials still required by a live workload" clause of FR-011 explicit for the immutable-revision case.
- **FR-018**: Any pull secret provisioned under a prior naming scheme (e.g. a legacy random-suffixed name) MUST be migrated on the next deploy/redeploy — the deterministic-named secret is ensured and the legacy secret is condemned for cleanup — without leaving the workload referencing a missing secret.
...
- **Auto-Provisioned Pull Secret**: A namespace-scoped Kubernetes secret of the image-pull type holding credentials only for the configured registry/registries that serve the deployment's own in-scope images (narrowed, least-privilege — not the full configured set), created/maintained by the system and referenced from generated workloads. It has a **stable, deterministic per-deployment name** (create-or-replaced in place across redeploys). Distinct from the existing build-time docker-config secret (which aggregates all configured registries) and from per-deployment sensitive-env secrets.
...
- **SC-007**: A deployment configured to scale to zero auto-resumes and serves a request on cold start in 100% of cases — every time it idles to zero and is hit again, and across intervening redeploys — with zero manual stop/start interventions (issue #387).
...
- **Secret lifecycle**: Auto-provisioned pull secrets are managed by the system and tracked by the existing disposable-resource lifecycle so they are reconciled/cleaned up rather than left to accumulate, subject to not removing secrets in active use. Each secret has a **stable per-deployment name** and is provisioned via **create-or-replace** so redeploys rewrite it in place; its tracking row is written directly in the stable state (never as an externally visible temporary row the scheduled cleaner could reap mid-provision). A secret is only condemned for cleanup when the deployment is removed or when migrating away from a legacy name — never while a live or scaled-to-zero revision still references it, and condemnation runs only after the CRD apply succeeded (FR-016 / FR-016a / FR-017 / FR-018).
