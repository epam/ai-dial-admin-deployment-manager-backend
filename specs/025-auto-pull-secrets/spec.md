...
- **FR-016a**: The pull secret MUST be written to the cluster in the same post-commit phase as the generated CRD (not before the deploy transaction commits), mirroring how the network policy is applied — so a rolled-back or failed deploy never leaves a mutated pull secret stranded from the workload that references it.
...
- **Secret lifecycle**: Auto-provisioned pull secrets are managed by the system and tracked by the existing disposable-resource lifecycle so they are reconciled/cleaned up rather than left to accumulate, subject to not removing secrets in active use. Each secret has a **stable per-deployment name** and is provisioned via **create-or-replace** so redeploys rewrite it in place; a secret is only condemned for cleanup when the deployment is removed or when migrating away from a legacy name — never while a live or scaled-to-zero revision still references it (FR-016 / FR-017 / FR-018).
