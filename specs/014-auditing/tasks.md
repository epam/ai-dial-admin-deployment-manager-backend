...
- [X] T020 [P] [US2] Create `PageRequestDto.java` in `src/main/java/com/epam/aidial/deployment/manager/web/dto/audit/` — request body DTO per `contracts/activities-api.md`: `@Min(0) int page` (default 0), `@Min(1) @Max(100) int size` (default 20), nested `@Valid SortDto sort` (field + direction), `@Valid List<FilterDto> filters`; `FilterDto` has `@NotBlank String field` and supports two shapes: equality filters (`field` + `value`) for string fields and range filters (`field` + `from`/`to`) for `epochTimestampMs`. Primitive `int` used instead of `Integer` to prevent NPE from explicit JSON `null` values (Jackson ignores null for primitives, preserving defaults)
