...
|----------|-------------|---------|-------------|
| `app.node-pools.label-key` | `NODE_POOL_LABEL_KEY` | _(empty)_ | Kubernetes node label key used to identify pools. Label selector is derived as `{labelKey: pool.name}`. Must be non-blank whenever `NODE_POOLS` is non-empty; may be blank when no pools are configured |
