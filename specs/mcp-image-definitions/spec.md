...
- Request DTO: `com.epam.aidial.deployment.manager.web.dto.McpImageDefinitionRequestDto` (extends `ImageDefinitionRequestDto`, adds nullable `transportType`)
- Response DTO: `com.epam.aidial.deployment.manager.web.dto.McpImageDefinitionDto` (extends `ImageDefinitionDto`, adds nullable `transportType`)
- Transport type enum: `com.epam.aidial.deployment.manager.web.dto.McpTransportTypeDto` (LOCAL, REMOTE)
- The `$type` discriminator value for MCP image definitions is `"mcp"` (registered on both `ImageDefinitionDto` and `ImageDefinitionRequestDto`)
