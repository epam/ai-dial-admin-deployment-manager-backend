...
4. **Given** a target revision id at which the deployment was not itself modified (i.e., the revision belongs to a different resource), **When** the administrator rolls back to it, **Then** the system resolves the snapshot to the deployment's state as of the latest applicable revision ≤ the target and the rollback succeeds — matching the deployment snapshot endpoint's lenient point-in-time semantics.
...
2. **Given** a target revision that precedes the whitelist's first recorded revision, **When** the administrator attempts to roll back to it, **Then** the system rejects the request with HTTP 404 and the whitelist is unchanged.
...
- **FR-002**: The system MUST reject the rollback with HTTP 404 when (a) the resource does not currently exist under the supplied identifier, or (b) the resource did not exist at the supplied revision. The system MUST resolve the target revision to the resource's state as of the latest applicable revision ≤ the target — matching the snapshot endpoint's lenient point-in-time semantics. Consequently, a target revision id that has no row in `revinfo` but is between revisions at which the resource was modified does not by itself trigger 404; rejection happens only when no restorable state exists at-or-before that revision.
