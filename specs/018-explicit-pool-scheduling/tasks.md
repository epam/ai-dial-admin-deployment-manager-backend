...
- [X] T029 [US4] Distinguish "absent" from "explicit null" in create/update payloads. Added a `transient boolean nodePoolFieldPresent` to both `CreateDeploymentRequestDto` (web DTO) and `CreateDeployment` (service model), with an overridden `setNodePool(...)` that flips the flag when Jackson invokes it. MapStruct propagates the flag DTO→service via property name; from `Deployment`→`CreateDeployment` the flag stays at its default (false), so imports trigger the cascade naturally. (Avoided the `JsonNullable` dependency from the plan — the flag mechanism is lighter and sufficient.)
- [X] T030 [US4] `DeploymentService.createDeployment(...)` invokes `applyCreateTimeNodePoolCascade(request)` before validation. Resolves and stamps the value when `nodePoolFieldPresent=false`; honours explicit values verbatim.
- [X] T031 [US4] `DeploymentService.updateDeployment(...)` preserves the existing stored `nodePool` when the request payload omits the field (`nodePoolFieldPresent=false`). Cascade never runs on update.
- [X] T032 [US4] `DeploymentMapper.toCreateCloneDeployment(...)` sets `nodePoolFieldPresent=true` on the cloned request so the duplicate carries the source's value verbatim (no cascade). FR-013 validation still rejects unknown names.
