...
```
DELETE /api/v1/images/builds/{id}
```
...
|---|---|---|---|
| `id` | UUID (string, canonical lowercase hex with dashes) | yes | The image definition whose in-progress build is to be stopped. |

### Request body

**None.** Per spec Clarification Q5 the image definition identifier is the complete contract; any request body is ignored.
...
| `404 Not Found` | `id` does not correspond to an existing image definition. | `ErrorView` |
...
- Side effects that MUST have happened before `204` is returned:
  ...
  2. `image_definition.build_status` is `BUILD_STOPPED` for the targeted `id`.
...
```json
{
  ...
  "status": 400,
  "error":  "Bad Request",
  "message": "Image build for '<id>' is not in progress (current status: <STATUS>)",
...
```json
{
  ...
  "status": 400,
  "error":  "Bad Request",
  "message": "Image build could not be stopped; build remains in BUILDING and may be retried",
  "traceparent": "00-<trace>-<span>-01"
}
```
...
6. **Cluster-side deletion failed**: `JobRunner.deleteJob` throws a simulated `KubernetesClientException`; DELETE returns `400` with the "could not be stopped" message; DB row remains `BUILDING`; no `BUILD_STOPPED` audit event emitted.
7. **Rebuild after stop**: After a successful stop, `POST /api/v1/images/builds` for the same `id` is accepted (FR-007).
