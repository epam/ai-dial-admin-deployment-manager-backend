...
**Decision**: Add `JobRunner.deleteJob(UUID groupId)` that:
1. Resolves target Job(s) via a Kubernetes label selector. At Job creation time `JobRunner.run` stamps every build Job with `image-definition-id=<groupId>` via `tagJobWithGroupId(...)` (applied before `saveK8sResources` so the label is present both on the live API object and on the persisted disposable-resource reference). Stop then calls `K8sClient.deleteJobsByLabelAndWait(buildNamespace, "image-definition-id", groupId, timeout)` which lists Jobs by that label in the hard-coded `app.build-namespace`. This replaces the originally planned DB-lookup approach (`DisposableResourceManager.getAllByGroupId` + filter by `K8sResourceKind.JOB`): label-based discovery keeps the Kubernetes layer self-contained (no DAO dependency for a K8s operation), and the label is the authoritative mapping on the cluster side — if a Job is present in the API server without a matching DB row, we still want to delete it, and conversely a stale DB row pointing to a Job that's already gone should not force us to call the API with a name that no longer exists.
2. Issues a Fabric8 `delete()` call with `DeletionPropagation.FOREGROUND` and `gracePeriod=0` for each matching Job so the Job's pods are cascaded.
3. Blocks on Fabric8's `waitUntilCondition(Objects::isNull, ...)` until each Job resource is removed from the API server, bounded by the hard-coded `image-build-stop-timeout-sec: 30` value in `application.yml`.
4. Propagates `KubernetesClientException` / `KubernetesClientTimeoutException` on API error or timeout; the orchestration layer wraps these in `ImageBuildStopFailedException`.

**Rolling-deploy note**: any build that was created before the `image-definition-id` label was introduced (i.e. an in-flight build at upgrade time) will not match the selector, so stop will find "no Jobs" and advance status to `BUILD_STOPPED` while the old Job runs to completion on its own. Acceptable — build Jobs are short-lived, so this is a one-time transition concern, not a steady-state issue.
...
- **Resolve the Job via `DisposableResourceManager.getAllByGroupId(groupId)`** (the original plan) — rejected after implementation review: forces the `kubernetes/` layer to depend on the DAO for something Kubernetes itself can answer via a label selector, and diverges from cluster truth whenever the DB row and the API server get out of sync. The label-selector approach keeps the Kubernetes layer self-contained and is consistent with how other K8s discovery is done in the codebase.
