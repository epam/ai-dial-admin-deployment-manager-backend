...
|---|---|---|---|
| `ImageDefinitionService.completeBuildSuccessfully(id, imageName, builtAt)` | `buildStatus != BUILD_STOPPED` after `findByIdForUpdate` | Set `buildStatus := BUILD_SUCCESSFUL`, `imageName := :imageName`, `builtAtMs := :builtAt`; `saveAndFlush` | void; log `DEBUG` and return when pre-check sees `BUILD_STOPPED` (pipeline lost race with stop) |
| `ImageDefinitionService.failBuild(id, errorLog)` | `buildStatus != BUILD_STOPPED` after `findByIdForUpdate` | Set `buildStatus := BUILD_FAILED`, append `errorLog`; `saveAndFlush` | void; same skip-on-race behavior |
...
Why the asymmetry: `stopBuild` only ever advances `BUILDING → BUILD_STOPPED` because the controller guards against every other starting state. `completeBuildSuccessfully` and `failBuild`, however, are also invoked by tests that shortcut directly from `NOT_BUILT` to a terminal state (legitimate test-only path in the pre-existing suite). A strict "must be BUILDING" guard on them would be a gratuitous behavior change. The one invariant this feature needs to preserve is that once `BUILD_STOPPED` is recorded it is not overwritten by the pipeline thread returning from `waitJob` — that narrow guard is what's installed.
