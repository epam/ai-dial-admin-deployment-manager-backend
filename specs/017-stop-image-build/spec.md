...
- **Stop requested as the build is completing**: The build naturally reaches success or failure at approximately the same moment a stop request arrives. The terminal state that wins must be deterministic — the system records whichever one took effect first. If the natural completion wins, the stop request is rejected with the same "build is not in progress" error that applies to any other non-running state (see FR-002), and the error payload MUST include the build's actual current terminal state so the client can refresh its view without a follow-up request.
