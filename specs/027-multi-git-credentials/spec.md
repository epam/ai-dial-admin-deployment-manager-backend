...
- What happens when two credential entries define the exact same scope (same host and same repository, or same host and same project path)? If they serve the same authentication type, the system MUST reject the configuration at startup with an error identifying the conflicting entries, rather than silently picking one. If one is SSH-only and the other HTTPS-only, both are accepted — the clone URL's protocol picks between them, so nothing is ambiguous.
- How is precedence resolved when two *different* parent domains could both match via subdomain inheritance (e.g. entries for `example.com` and `b.example.com`, for a repository on `a.b.example.com`)? The closest (longest) configured host wins, before any path specificity is considered — so `b.example.com` with a broad path beats `example.com` with an exact repository path.
- What if a `path` is written in different casing than the repository URL (e.g. `Team/Service-A` vs. `team/service-a`)? It does not match — paths are compared case-sensitively — and resolution falls back to a less specific entry, or to no credentials.
...
- **FR-002b**: System MUST compare hosts case-insensitively (independent of the JVM's default locale) and paths case-sensitively, consistent with how git hosting providers treat each.
- **FR-003**: System MUST, when resolving credentials for a repository URL, select the configured credential whose scope most specifically matches that URL, ranked as follows (most specific first): (1) any match on the URL's exact host, ahead of any parent-domain (subdomain-inherited) match; (2) among parent-domain matches, the closest — longest — configured host, ahead of any farther parent domain, regardless of path specificity; (3) within the same configured host, an exact repository match, then a project/group match, then a domain-wide match, with the longest matching prefix winning among nested project/group paths.
...
- **FR-005**: System MUST validate configuration at startup and reject it with an error identifying the conflicting entries when two or more credential entries define the identical scope for the same host **and the same authentication type** (SSH, or HTTPS/HTTP). Entries sharing a scope with different authentication types MUST be accepted, since the repository URL's protocol selects between them.
...
- A `path` scope governs credential *selection*, not access enforcement: once resolved, the credential is written to the build's credential store for the whole host, so it remains usable for other repositories on that host from inside the same build (unchanged from the single-credential-per-host behavior).
