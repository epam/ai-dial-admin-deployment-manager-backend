...
- [X] T006 Add cross-entry duplicate-scope validation to `GitConfiguration` (FR-005, research D4): after parsing all entries, group by `(GitScopeUtils.normalizeHost(host), normalized-path-or-null, authentication type)`; if any group has more than one entry, throw `IllegalArgumentException` naming the conflicting `host`/`path`, the authentication type and the position of each conflicting entry in the source list, in `src/main/java/com/epam/aidial/deployment/manager/configuration/GitConfiguration.java` (depends on: T004, T005)
...
- [X] T016a [P] [US3] `GitServiceTest`: among two parent-domain matches, the closest (longest) configured host wins ahead of path specificity (FR-003, research D3)
- [X] T016b [P] [US3] `GitServiceTest`/`GitConfigurationTest`: an SSH entry and an HTTPS entry may share one `(host, path)` scope — startup accepts them and each clone protocol resolves to its own entry, while two entries of the same authentication type at one scope still fail startup (FR-005, research D4)
