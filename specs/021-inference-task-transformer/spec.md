...
- **Transformer-scheduling co-location**: pool scheduling primitives selected via the deployment's `nodePoolId` are projected onto the transformer block identically to the predictor (see 019-explicit-pool-scheduling FR-006 and the kubernetes-manifests capability spec), so both pods of a chained pair land on the same node pool.
