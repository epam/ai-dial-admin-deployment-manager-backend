...
| Detection returns `TEXT_CLASSIFICATION` and `INFERENCE_TEXT_CLASSIFICATION_TRANSFORMER_IMAGE` is unset | 500 | "Cannot deploy a text-classification inference deployment: required configuration property 'app.inference.text-classification-transformer.image' (env INFERENCE_TEXT_CLASSIFICATION_TRANSFORMER_IMAGE) is not set." |
