...
- **FR-007**: When a client opens the build log stream for a completed build (replay mode), all stored log lines MUST be emitted first (in array order), followed by all captured domain events (ordered by observation time ascending), followed by the terminal status event and automatic stream closure.
...
- Q: In what order should domain events be replayed for a completed build stream? → A: Ordered by `observed_at` ascending — the receive time recorded when the flow was processed
