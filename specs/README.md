...
| [023-model-metrics-api](023-model-metrics-api/spec.md) | Implemented | N/A — creates new capability model-metrics | Unified model metrics API (PoC): live snapshot endpoint scraping vLLM/TGI/SGLang/NIM metrics via the kube API-server pod proxy, normalized to an engine-neutral schema with graceful degradation; verified live on dev-cluster vLLM + LLM-NIM deployments |
...
| [model-metrics](model-metrics/spec.md) | Implemented | Unified model metrics API — live snapshot of engine serving/operational metrics (vLLM/TGI/SGLang/NIM) normalized to one schema; pod-proxy scrape, lifetime window, graceful degradation |
