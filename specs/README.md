...
| [023-deployment-metrics-api](023-deployment-metrics-api/spec.md) | Implemented | N/A — creates new capability deployment-metrics | Unified deployment metrics API: live snapshot endpoint reporting replica counts and per-pod CPU/memory for any deployment type, plus vLLM/TGI/SGLang serving metrics scraped via the kube API-server pod proxy for inference deployments, normalized to an engine-neutral schema with graceful degradation |
