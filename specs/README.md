...
| [023-model-metrics-api](023-model-metrics-api/spec.md) | Implemented | N/A — creates new capability model-metrics | Unified deployment metrics API (PoC): live snapshot endpoint reporting replica counts and per-pod CPU/memory for any deployment type, plus vLLM/TGI/SGLang serving metrics scraped via the kube API-server pod proxy for inference deployments, normalized to an engine-neutral schema with graceful degradation; verified live on a dev-cluster vLLM deployment |
