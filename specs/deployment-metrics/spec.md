...
The system SHALL expose `GET /api/v1/deployments/{id}/metrics` returning a live unified metrics snapshot for a deployment of any type. Resource metrics (replica counts and per-pod CPU/memory) are reported for every type; serving-quality metrics are scraped for INFERENCE deployments and marked unavailable for all others. An unknown id yields HTTP 404 (`ErrorView`, per `api-conventions`). For INFERENCE deployments the Ready predictor pod is scraped per request and named in `scrapedPod`; for a chained KServe Python ModelServer deployment the Ready transformer pod is additionally scraped to combine pre/post-processing latency with predictor inference latency, but only when the transformer advertises a Prometheus endpoint (the `prometheus.kserve.io/port` annotation KServe stamps only on pods exposing one) — otherwise it is skipped and the snapshot degrades to predictor-only. Serving and operational metrics therefore reflect that single scraped replica (named in `scrapedPod`), not an aggregate across replicas — for `replicas > 1` they are single-replica figures (per-deployment aggregation is out of scope). The replica is selected deterministically (lowest pod name) so it stays stable across polls. When component-labelled pods exist but no predictor is Ready (e.g. a chained InferenceService still starting), the serving/operational blocks are marked unavailable with that reason rather than scraping the transformer.
...
- **THEN** the detected engine is `KSERVE_MODELSERVER`, the `serving` block reports the engine-neutral signals (`requestLatency` distribution and `requestsPerSecond` throughput) while the generative-only fields stay null, and the `operational` block reports an end-to-end latency; for a chained deployment whose transformer advertises a Prometheus endpoint the end-to-end latency combines the transformer's pre/post-processing with the predictor's inference (its mean is the sum of the per-stage means; cross-pod percentiles are null) — when the transformer exposes no endpoint the end-to-end latency is the predictor's predict histogram alone — and the predictor pod is named in `scrapedPod`
