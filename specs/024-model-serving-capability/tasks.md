...
- [x] T006 [P] Create Flyway migration `src/main/resources/db/migration/H2/V1.59__AddInferenceTaskColumn.sql` — `ALTER TABLE inference_deployment ADD inference_task VARCHAR(32);` on the main table plus the `_aud` audit column at `VARCHAR(255)` (nullable)
