...
- Build-time `config.json` keys (`RegistryService.dockerConfig()`, mounted into build/copy jobs — not an image-pull secret):
  - Key shape: `https://<host>/v2` for every registry, **except** Docker Hub aliases (`docker.io`, `index.docker.io`, `registry-1.docker.io`) which use the legacy key `https://index.docker.io/v1/`.
  - Why the legacy key matters: `docker/cli` (and therefore BuildKit's authprovider, used by MCP LOCAL transport) hardcodes `https://index.docker.io/v1/` for the Hub Official index — a `/v2`-shaped key for `docker.io` is silently ignored there. Skopeo / containers-image normalizes both forms transparently, so the legacy key is harmless for the Skopeo copy path.
  - Collision behaviour: multiple entries can collapse to the same auth key — both across Hub aliases (e.g. one entry for `docker.io` and another for `index.docker.io`) and across the main registry vs. a trusted entry pointing at the same host. Only the last set of credentials survives; `RegistryService` logs a `WARN` when this happens so operators can spot the misconfiguration in logs.
  - Contrast with deploy-time image-pull secrets: `RegistryService.dockerConfigForImages(...)` keys entries with the **bare registry host** (no scheme, no `/v2`; Docker Hub keeps the legacy key) because the kubelet credential keyring path-matches keys against the image reference — see the "Auto-provisioned imagePullSecrets" requirement in `specs/kubernetes-manifests/spec.md`.
- `DockerRegistryClient.getRegistryClient()` (jib-core path, used for in-manager-process image inspection like inheriting a source image's entrypoint during wrapper builds):
  ...
  - Once credentials are matched, the auth handshake tries bearer first via jib's `doPullBearerAuth()`. Modern registries (Docker Hub, ACR, GHCR, GAR, ECR, …) return a `WWW-Authenticate: Bearer …` challenge and reject a hardcoded `Authorization: Basic …` header on the registry API; `doPullBearerAuth()` performs the token-endpoint round-trip with the credentials. When the server instead advertises `WWW-Authenticate: Basic` (self-hosted Distribution v2 + htpasswd, Harbor basic mode, Artifactory without bearer, …), jib's `doPullBearerAuth()` returns `false` and leaves `Authorization` unset — `DockerRegistryClient` then explicitly calls `configureBasicAuth()` as a fallback so credentials are still sent on the subsequent manifest/blob pulls.
