...
**Created**: 2026-05-27
**Status**: Implemented (reworked post-initial-flip: HF Hub re-fetch dropped from update path, rolling-update CNP refresh added, kube-dns emitted for chained pairs under locked-down `allowedDomains`, fail-fast on absent cluster service)
...
- **Update-path signal source**: `updateCiliumNetworkPolicy(id)` (invoked on `allowedDomains` / `containerPort` edits) sources the chained signal by reading the existing cluster `InferenceService` and checking for a `transformer` block — not by re-running HuggingFace detection. Topology flips reach the policy via `rollingUpdate`, which already runs manifest generation once and reuses the signal from that single computation. The cluster read adds one Kubernetes API call per `allowedDomains` edit (a new dependency vs. the pre-feature path that derived no signal at all); if the live `InferenceService` is absent the endpoint fails fast with `DeploymentException` rather than emitting a downgraded policy. See Risks for the failure-mode tradeoff.
...
- **Post-commit CNP drift on `allowedDomains` / `containerPort` edits**: The CNP refresh runs in `afterCommit` so the K8s call doesn't hold a pooled DB connection across kube-apiserver latency. If the K8s write fails after the DB commit, the persisted state and the live policy drift — the DB reflects the new `allowedDomains` / `containerPort` while the cluster CNP still reflects the old. The drift is logged at WARN (`"CNP refresh failed post-commit for deployment '…' (… drift possible until re-sync)"`); operators can re-sync by re-submitting the PATCH or by any subsequent `deploy` / `rollingUpdate` on the deployment. This matches how `rollingUpdate` and `deploy` in `AbstractDeploymentManager` already handle post-commit K8s failures: Spring's `TransactionSynchronizationUtils.invokeAfterCommit` swallows exceptions thrown from `afterCommit` callbacks (the rethrow in the catch block is for log clarity, not HTTP propagation). Closing the silent-failure window codebase-wide is out of scope for this spec — it would require reshaping every `afterCommit`-based K8s mutation. Alternative considered and rejected: synchronous CNP write inside the `@Transactional` boundary. That preserves atomicity but holds a DB connection for the duration of the K8s read+write (one cluster read + one CNP write on the inference path), which the spec-022 author chose to avoid (commit `67971160`).
