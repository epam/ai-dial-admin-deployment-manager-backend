...
- [ ] T007 [US1] In `src/main/java/com/epam/aidial/deployment/manager/service/pipeline/specification/CiliumNetworkPolicyCreator.java`, implement the chained-mode emission inside the new 6-arg `create(...)`. When `chainedTransformer=true`:
...
- [ ] T011 [P] [US1] In `src/test/java/com/epam/aidial/deployment/manager/service/deployment/InferenceDeploymentManagerTest.java`, add:
...
- [ ] T012 [P] [US2] In `src/test/java/com/epam/aidial/deployment/manager/service/pipeline/specification/CiliumNetworkPolicyCreatorTest.java`, add `shouldProduceIdenticalPolicyTo5ArgOverload_whenChainedFalse`: invoke both `create(ns, labelName, labelValue, domains, ports)` and `create(ns, labelName, labelValue, domains, ports, false)` with several fixture combinations (empty `allowedDomains`, populated `allowedDomains`, null `ports`, populated `ports`) and assert deep equality of the resulting `CiliumNetworkPolicy` objects. This is the SC-002 byte-equivalence canary.
- [ ] T013 [P] [US2] In `src/test/java/com/epam/aidial/deployment/manager/service/deployment/KnativeDeploymentManagerTest.java`, `NimDeploymentManagerTest.java`, and `src/test/java/com/epam/aidial/deployment/manager/kubernetes/JobRunnerTest.java`, verify the 5-arg `create(...)` overload is still the call shape (`verify(ciliumNetworkPolicyCreator).create(eq(NAMESPACE), anyString(), eq(SERVICE_NAME), anyList(), any())` — no boolean arg). If Mockito strict-stubbing complains because the 6-arg overload exists, switch the verification to use `argument captors` matching only the 5 relevant arguments. No production-code change in this task; failing tests indicate an accidental call-site regression from Phase 2.
- [ ] T014 [P] [US2] In `src/test/java/com/epam/aidial/deployment/manager/service/deployment/InferenceDeploymentManagerTest.java`, add `shouldEmitBaselinePolicy_whenDetectionReturnsNone` — covers a non-classification HF deployment + a non-HF source. Asserts the generated policy contains no chained-mode rules (no `serving.kserve.io/inferenceservice` ingress fromEndpoint, no `8080/TCP` literal beyond what container-port resolution supplies).
