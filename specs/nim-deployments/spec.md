...
- **WHEN** a NIM deployment is deployed
- **THEN** the generated NIMService has `inferencePlatform: kserve`, `expose.router` set to empty object, no `expose.ingress`, and Knative autoscaling annotations on **`spec.annotations`** (not `metadata.annotations` — the NIM operator propagates only `spec.annotations` to the KServe InferenceService, see [NVIDIA/k8s-nim-operator#829](https://github.com/NVIDIA/k8s-nim-operator/issues/829)): `min-scale`, `max-scale`, and `initial-scale` are always set (default `1/1/1` when no `Scaling` is provided, otherwise derived from the `Scaling` with `initial-scale = Math.max(minReplicas, 1)`); `autoscaling.knative.dev/class`, `autoscaling.knative.dev/metric`, and `autoscaling.knative.dev/target` are set only when a `Scaling` with `ACTIVE_REQUESTS` strategy is provided (otherwise Knative cluster defaults apply). The `serving.knative.dev/progress-deadline` annotation is likewise placed on `spec.annotations` in this mode.

> **Upstream limitation** (NIM operator ≤ current, KServe serverless): `min-scale`/`max-scale` are emitted for intent but never reach the Knative Revision — KServe filters them via its `ServiceAnnotationDisallowedList` and derives replica bounds from predictor `minReplicas`/`maxReplicas`, which the NIM operator does not set in serverless mode. The Revision effectively runs with `min-scale=1` and unbounded `max-scale`; scale-to-zero is not achievable for NIM KServe deployments. The generator logs a warning when a `Scaling` requests other bounds.
...
- Related specs: `deployments` (base + lifecycle), `kubernetes-manifests`, `deployment-metrics` (live resource-metrics snapshot — replica counts and per-pod CPU/memory via `GET /api/v1/deployments/{id}/metrics`)
