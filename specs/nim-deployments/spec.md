...
- Related specs: `deployments` (base + lifecycle), `kubernetes-manifests`, `model-metrics` (live resource-metrics snapshot — replica counts and per-pod CPU/memory via `GET /api/v1/deployments/{id}/metrics`)
