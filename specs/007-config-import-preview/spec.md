...
- **FR-008**: The system MUST treat the `globalImageBuildDomainWhitelist` as a single unit: one `ImportComponentDto<List<String>>` where `prev` is the current full whitelist and `next` is the incoming full whitelist (subject to the same conflict resolution semantics). No component is emitted when the incoming list is empty or absent.
