...
class ImageDefinitionControllerTest extends AbstractControllerNoneSecureTest {
    ...
    void testGetGroupedImageDefinitionViewsByType() throws Exception {
        ...
        var pageable = PageRequest.of(0, 20);
        var page = new PageImpl<>(models, pageable, models.size());
        doReturn(page).when(imageDefinitionService).getImageDefinitionViewsByType(eq(DeploymentTypeDto.MCP), any(Pageable.class));
        ...
        verify(imageDefinitionService).getImageDefinitionViewsByType(eq(DeploymentTypeDto.MCP), any(Pageable.class));
    }
