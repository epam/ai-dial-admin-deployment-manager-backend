...
insert into domain_whitelist (id, allowed_domains, updated_at) values (RANDOM_UUID(), '[]' FORMAT JSON, CURRENT_TIMESTAMP(3));

-- Add 'allowed_domains' column to 'image_definition' table
alter table image_definition
    add column if not exists allowed_domains json not null default '[]' FORMAT JSON;
