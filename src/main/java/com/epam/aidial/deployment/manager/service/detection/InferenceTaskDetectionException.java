...
/**
 * Base class for failures raised by {@link InferenceTaskDetector} when a HuggingFace model
 * cannot be successfully classified or its metadata cannot produce a usable
 * {@code id2Label} map. Concrete subclasses convey the specific cause; the web handler
 * maps each subclass to the HTTP status documented in
