...
| `app.registry.auto-pull-secret-enabled`   | `AUTO_PULL_SECRET_ENABLED`   | `true`                 | No                                                | -            | When `true`, the service auto-provisions a `kubernetes.io/dockerconfigjson` pull secret and wires it into a deployment's generated workload (`imagePullSecrets`) whenever an in-scope image — an image-based deployment's image or a chained inference transformer image — comes from a credentialed registry configured above (the primary registry or a trusted-private registry with `BASIC` auth). Images from public/unconfigured registries are untouched. Set to `false` to disable and manage pull secrets manually. |
...
| Property                        | Environment Variable        | Default Value | Required | Applied when | Description                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| ------------------------------- | --------------------------- | ------------- | -------- | ------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `app.git.trusted-private-repos` | `TRUSTED_PRIVATE_GIT_REPOS` | -             | No       | -            | JSON array of Git repository configuration objects. Each object must have at least `"host"` (the Git repository host), and may include `"path"` (optional; scopes the entry to a specific repository or project/group path under `host` instead of the whole domain — see below), `"protocol"` (`"https"` or `"http"`, default: `"https"`), `"user"`, `"password"`, `"token"`, `"sshKeyPath"`, and `"sshKnownHostsPath"`. The `"protocol"` field is only applicable for HTTPS/HTTP authentication (when using `"user"`/`"password"` or `"token"`), and is not applicable when using SSH key authentication (`"sshKeyPath"`). For SSH authentication, `"sshKeyPath"` and `"sshKnownHostsPath"` must be file paths (not file contents). The files are read at application startup and the application will fail to start if the files are not found. Multiple entries may share the same `host` as long as they stay distinguishable: either their `path` differs (omitted on at most one of them, for a domain-wide fallback), or they use different authentication types — one SSH entry (`sshKeyPath`) and one HTTPS/HTTP entry (`user`/`token`) may share the same `host` and `path`, since SSH URLs only ever resolve to SSH entries and HTTPS/HTTP URLs only ever resolve to `user`/`token` entries. The application fails to start if two entries resolve to the identical `(host, path)` scope for the same authentication type. When cloning a repository, the most specific matching entry wins: an exact repository `path` beats a project/group-prefix `path`, which beats a domain-wide entry (no `path`), and an exact host match always beats a match inherited from a parent domain (e.g. a `host` of `example.com` also matching `git.example.com`) regardless of `path` specificity; among parent-domain matches, the closest (longest) configured `host` wins before `path` specificity is considered (e.g. for `a.b.example.com/team/service-a`, an entry for `b.example.com` + `path: "team"` beats one for `example.com` + `path: "team/service-a"`). `host` is matched case-insensitively, but `path` is matched **case-sensitively** — write `path` with exactly the casing used in the repository URL. Note that `path` selects *which* credential is mounted for a build; it is not an access boundary — the selected credential is offered to every repository on that host inside the build container (a submodule under a different path included). Example: `[{"host":"git.example.com","path":"team/service-a","user":"user1","token":"repo-token"},{"host":"git.example.com","path":"team","user":"user1","token":"project-token"},{"host":"git.example.com","protocol":"https","user":"user1","password":"pass1"},{"host":"git.private.com","sshKeyPath":"/path/to/id_rsa","sshKnownHostsPath":"/path/to/known_hosts"}]` These are trusted private Git repositories from which the service is allowed to clone code during build operations. |

> **Breaking change on upgrade:** startup now fails if two entries resolve to the identical `(host, path)` scope for the same authentication type (see above). This was never enforced before — a configuration with two or more redundant entries for the same host (no `path`, same authentication type) previously started and silently used the first matching entry. Before upgrading, check `TRUSTED_PRIVATE_GIT_REPOS` for such duplicates and remove the redundant entry.
...
#### Deployment Metrics Scrape Configuration

Powers `GET /api/v1/deployments/{id}/metrics` — the on-demand live metrics snapshot for **any** deployment type (see `specs/deployment-metrics/spec.md`). Resource metrics (replica counts and per-pod CPU/memory) are reported for every deployment type. Engine-specific model metrics — both serving-quality (TTFT, inter-token latency, tokens/second, queue depth, KV-cache usage, …) and operational (request error ratio, end-to-end latency) — are additionally scraped only for **INFERENCE** deployments; for all other types (MCP/adapter/application/interceptor and **NIM**) the serving and operational blocks are reported unavailable with a reason, so NIM model metrics are not supported. For INFERENCE deployments the engine's Prometheus `/metrics` is read from the Ready predictor pod through the Kubernetes API-server pod proxy; collection is request-triggered only (never background polling). GPU metrics (per-pod utilization and framebuffer memory used/total) are read for deployments that request `nvidia.com/gpu`, from the NVIDIA **DCGM exporter** DaemonSet — a **cluster prerequisite** deployed on GPU node pools — by scraping only the exporter pods co-located on the deployment's nodes through the same pod proxy. Requires read-only RBAC on `pods/proxy` (and `metrics.k8s.io` pods when the resource-usage block is enabled); when GPU metrics are enabled it additionally requires read-only RBAC on `pods` and `pods/proxy` in the DCGM exporter namespace (default `gpu-operator`). A missing/unreachable exporter, or a non-GPU deployment, degrades gracefully (the `resources.gpu` block is reported unavailable with a reason — never an error).
...
| `app.metrics.scrape.timeout-ms`              | `METRICS_SCRAPE_TIMEOUT_MS`                | `10000`       | No       | -                             | Per-scrape budget (ms) for reading a pod's `/metrics` through the API-server proxy (also bounds each DCGM exporter scrape). On timeout the affected block degrades gracefully (never an error). Must be positive — a non-positive value fails startup. |
| `app.metrics.scrape.cache-ttl-ms`            | `METRICS_SCRAPE_CACHE_TTL_MS`              | `5000`        | No       | -                             | Response cache TTL (ms) per deployment id — bounds API-server load under rapid repeated requests (e.g. an auto-refreshing UI panel). Must be positive — a non-positive value fails startup. |
...
| `app.metrics.scrape.gpu.enabled`             | `METRICS_SCRAPE_GPU_ENABLED`               | `true`        | No       | -                             | Enables the per-pod GPU block (utilization + framebuffer memory used/total) sourced from the DCGM exporter, for deployments that request `nvidia.com/gpu`. Degrades gracefully when the exporter is absent. |
| `app.metrics.scrape.gpu.namespace`           | `METRICS_SCRAPE_GPU_DCGM_NAMESPACE`        | `gpu-operator` | No | GPU block enabled             | Namespace the DCGM exporter DaemonSet runs in.                                                                                                          |
| `app.metrics.scrape.gpu.pod-label-selector`  | `METRICS_SCRAPE_GPU_DCGM_POD_SELECTOR`     | `app=nvidia-dcgm-exporter` | No | GPU block enabled        | Label selector (`k=v,k2=v2`) identifying the DCGM exporter pods.                                                                                        |
| `app.metrics.scrape.gpu.port`                | `METRICS_SCRAPE_GPU_DCGM_PORT`             | `9400`        | No       | GPU block enabled             | Port the DCGM exporter serves Prometheus metrics on. Must be positive — a non-positive value fails startup.                                             |
| `app.metrics.scrape.gpu.metrics-path`        | `METRICS_SCRAPE_GPU_DCGM_METRICS_PATH`     | `/metrics`    | No       | GPU block enabled             | HTTP path of the DCGM exporter's Prometheus endpoint.                                                                                                   |
| `app.metrics.scrape.gpu.pod-label`           | `METRICS_SCRAPE_GPU_DCGM_POD_LABEL`        | `pod`         | No       | GPU block enabled             | DCGM series label carrying the owning pod name (used to attribute GPUs to pods). Direct exporter scrapes expose the raw `pod` label.                    |
| `app.metrics.scrape.gpu.namespace-label`     | `METRICS_SCRAPE_GPU_DCGM_NAMESPACE_LABEL`  | `namespace`   | No       | GPU block enabled             | DCGM series label carrying the owning pod's namespace.                                                                                                  |
...
| Property                                                                | Environment Variable                                              | Default Value | Required                                          | Applied when                                | Description                                                                                                                  |
|-------------------------------------------------------------------------|-------------------------------------------------------------------|---------------|---------------------------------------------------|---------------------------------------------|------------------------------------------------------------------------------------------------------------------------------|
| `app.text-classification-transformer-container-config`                            | -                                                                              | (see below)        | -                                                 | Chained manifest generation                 | Full Kubernetes Container spec used as the template for the chained text-classification transformer container. Mirrors the `app.knative-service-container-config` pattern; any standard K8s Container field (env, imagePullPolicy, securityContext, …) can be added here. Per-deployment bits (`ID2LABEL` env, `--model_name=<name>` arg) are layered on top by the manifest generator. |
| `app.text-classification-transformer-container-config.image`                      | `INFERENCE_TEXT_CLASSIFICATION_TRANSFORMER_IMAGE`                              | -                  | Yes for chained deployments                       | Deploying an inference whose model is detected as text-classification | Container image. Deploy fails fast with HTTP 5xx if unset for chained mode. |
| `app.text-classification-transformer-container-config.resources.requests.cpu`     | `INFERENCE_TEXT_CLASSIFICATION_TRANSFORMER_CPU_REQUEST`                        | `100m`             | No                                                | Chained manifest generation                 | CPU request for the transformer container.                                                                                   |
| `app.text-classification-transformer-container-config.resources.requests.memory`  | `INFERENCE_TEXT_CLASSIFICATION_TRANSFORMER_MEMORY_REQUEST`                     | `256Mi`            | No                                                | Chained manifest generation                 | Memory request for the transformer container.                                                                                |
| `app.text-classification-transformer-container-config.resources.limits.cpu`       | `INFERENCE_TEXT_CLASSIFICATION_TRANSFORMER_CPU_LIMIT`                          | `500m`             | No                                                | Chained manifest generation                 | CPU limit for the transformer container.                                                                                     |
