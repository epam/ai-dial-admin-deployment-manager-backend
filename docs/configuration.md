...
| `app.nim-service-expose-ingress-config.enabled`               | `APP_NIMSERVICEEXPOSEINGRESSCONFIG_ENABLED`               | `true`  | No | NIM deploy (legacy mode with external URL) | Enable or disable Ingress creation for externally exposed NIM services. Overridable via env var (Spring relaxed binding). |
